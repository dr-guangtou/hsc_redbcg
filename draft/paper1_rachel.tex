...
\begin{abstract} 
    ...
    These stellar halos are a key fossil record of galaxy assembly history. Using data that is both wide (${\sim}100$ deg$^2$) and deep ($>28.5$ \sb{} in 
    the $i$-band), we present a systematic study of the stellar halos of  a sample of more than $7000$ galaxies at $0.3 < z < 0.5$. 
Our study is based on high-quality (0.6\asec\ seeing) imaging data from the Hyper 
    Suprime-Cam (HSC) survey, which reaches ${\sim}4$ magnitudes fainter than SDSS and enables us to directly trace mass profiles to 100 kpc 
    without resorting to stacking. Shallow images and/or over-simplified parametric models cannot capture the extended stellar halo and result in underestimated stellar masses.
    Our extended profiles reveal a number of mass-dependent trends. The outer parts of our galaxies have higher ellipticity, and this ellipticity increases with stellar mass, but we find no difference in color gradients with mass. As in previous work, we see a clear trend towards more extended outer profiles in more massive galaxies. Thus, when this extended light is not properly accounted for, the stellar mass function is systematically underestimated at the highest masses. Our results are broadly consistent with the two phase formation scenario of massive galaxies in which outer envelopes are built up at late times from minor merging. We provide surface mass surface mass density profiles in a convenient tabulated format to facilitate comparisons with predictions from numerical simulations of galaxy formation.\end{abstract}
...
%    in the outskirt increasing with total stellar mass (xxxxxxxxx).
...
%% ------------------------------------------------------------------------------------ %% 
    \begin{figure*}
        ...
        \caption{
            ...
            large radii.
\rachel{Should replace the quote marks with arcsec symbols in the scale on these plots.}
}
...
\section{Introduction}
    ...
    halos  (e.g. \citealt{Szomoru2012, Patel2013}). According to this formation mechanism, minor mergers dominate the mass assembly of massive galaxies at late times (e.g. \citealt{Hilz2012, Hilz2013, Oogi2013, Bedorf2013, Laporte2013}).
    ...
    accreted stars increases significantly and reaches $f_{\mathrm{accreted}}>0.5$ at \logms{}$>11.5$ (\citealt{RodriguezGomez2016}). \rachel{Given that Illustris gets the SMF totally wrong at high stellar mass, is there a reason we should be quoting their results?  Perhaps better to wait for Illustris-TNG?  Or give some caveat here that it's not clear that the physics that shapes the most massive galaxies is a realistic implementation?}
%% ------------------------------------------------------------------------------------ %% 
    
    Given the success of this two-phase scenario in explaining trends like overall size 
    growth, it is time to confront this model with additional observations,  in particular, the detailed surface mass density profiles of low redshift massive galaxies. Early studies based on one-dimensional light profiles suggested that the profiles of nearby ETGs are well described by single-\ser{} profiles (e.g. \citealt{Kormendy2009}; except for the
    most central region) and that the \ser{} index increases with total luminosity 
    (e.g. \citealt{Graham2013}). However,  later work revealed a more complicated  situation  showing  that  ETGs  belong to two famillies, those that follow single-\ser{} law, versus those that significant deviation from the single-\ser{} profile \citep{Schombert2015}. \rachel{You should say what differed in these later studies; e.g., use of deeper/better data?} Two-dimensional analyses have found that the stellar distributions of massive ETGs are often better fit by multiple-component models 
    ...
    (e.g. \citealt{Bernardi2013, DSouza2014}) as well as to quantify the amplitude and scatter among outer envelopes (e.g. \citealt{Capaccioli2015, Iodice2016, 
    Iodice2017}).
    
    Until now,  studies of the profiles of massive galaxies have been  either  been conducted  using large samples but with shallow imaging, for example the Sloan Digital Sky Survey (SDSS; e.g. \citealt{SDSSDR7, SDSSDR12}), or with deeper imaging  but smaller sample sizes (e.g cite CLASH AND FRONTIER FIELDS??). In this paper, we take advantage of new high-quality (median  seeing of
 ...
 %   $0.3 < z < 0.5$ using ${\sim} 100$ deg$^2$ of data from the HSC wide layer. 
    ...
    Section \ref{sec:summary} presents our summary and conclusions. \rachel{Outline does not mention section 5.}
...
%% ------------------------------------------------------------------------------------ %% 
    \begin{figure*}
        ...
        \caption{
            ...
            affected by seeing. \rachel{`most strongly affected by seeing'?  It's not the case that the seeing is totally irrelevant outside this region.  The `most affect' in the sentence below does not quite make sense either.}
...
\rachel{Need to cite the HSC survey camera paper, Miyazaki et al in prep.}
...
\rachel{Need to say how this relates to the release that is available to the public and described in the HSC DR1 paper, for the sake of reproducibility.}
The regions covered by this release overlap with a number of spectroscopic surveys 
    ...
    galaxies out to their very outskirts.  \rachel{This language seems a bit too informal for a journal article.  Perhaps `\dots represents an ideal dataset to perform statistical studies\dots'?}   
...
\rachel{You do not seem to explicitly calculate luminosities, but rather to make a flux (not luminosity) cut.  So should we call it a `broad flux cut'?}
Based on \citet{Leauthaud2016}, $i_{\mathrm{SDSS, cModel}} \leq 21.0$ mag can 
    ...
    the \texttt{WIDE} survey in the $i$-band. \rachel{As defined in DR1 paper?}
...
\rachel{You require it to be a redmapper cluster member?  Or do you mean it must be part of the redmagic galaxy sample?  I do not fully understand the logic in using redmapper rather than redmagic.}
We match the \texttt{hscPho} sample with a spec-$z$ catalog compiled by the HSC 
    ...
    flag that indicates secure redshifts. \rachel{Is there a paper or a note at the data release page that we could reference so readers can check out how this was defined?}}.  
...
\rachel{This is only of order 1\% of the original sample.  Do we understand why?  Of particular concern are cuts that affect survey homogeneity; e.g., in the area covered by GAMA vs.\ the area not covered by GAMA, might the sample look different?  What will that do to this study?}
...
\section{Measurements of 1-D Surface Brightness Profiles}
    ...
    ($r>100$ kpc). \rachel{Here too I think we have to be careful about how we say this.  The PSF is not irrelevant outside of the inner part of the profile.  It's just less important.  Perhaps say `\dots where we are less sensitive to the PSF or the background subtraction\dots'}
