...
\section{Sample Selection and Data Reduction}
    ...
    the full depth in the Wide field. The data are reduced by \texttt{hscPipe 4.0.2}, a derivative of the 
    ...
    We use $i$-band images for extracting surface brightness profiles. HSC $i$-band images are typically 3--4 mag deeper than SDSS (spell out here and reference ?????)  and have superb seeing conditions (mean $i$-band seeing has FWHM$=0.6$\asec{}).
    
    In Paper~I, we select a sample of xxxxx(give number) massive galaxies with spectroscopic 
    redshifts or reliable ``red-sequence'' photometric redshifts (\citealt{Rykoff2014}) 
    at $0.3<z<0.5$. Within this redshift range, we have a large enough volume (xxx give the number here) to sample the galaxy mass function above $M-*>11.6$, we can spatially resolve galaxies profiles to $\sim 5$ 
    kpc ($1.0^{\arcsec}$ corresponds to 4.4 and 6.1 kpc at 
    $z=0.3$ and 0.5), and we can safely ignore structural evolution over our redshift range (give time span in Gyr xxx).
    
    We derive $i$-band surface brightness profiles out to 100 kpc after carefully 
    masking out surrounding neighbors and accounting for the subtraction of the background light. We use the broadband spectral energy distributions (SED) fitting code 
    \texttt{iSEDFit}\footnote{http://www.sos.siena.edu/~jmoustakas/isedfit/} 
    (\citealt{Moustakas13}) to measure \m2l{} ratios and $k$--corrections using five-band forced \cmodel{} magnitudes from \texttt{hscPipe}. We assume a Chabrier IMF \citep{Chabrier2003}, the Flexible Stellar Population 
    Synthesis models\footnote{http://scholar.harvard.edu/cconroy/sps-models}
    (FSPS; \texttt{v2.4}; \citealt{FSPS}, \citealt{Conroy2010}), a Calzetti extinction law \citep{Calzetti2000}, and a simple delayed-$\tau$ model for star 
    formation histories (SFH). Using HSC data, we can measure the mass profiles of massive galaxies to $\sim 100$ kpc and we integrate these profiles within isophotal elliptical apertures at different physical radii. As explained in Paper I, here focus on the two following metric masses:
        
    \begin{itemize}
    
        \item stellar mass within 10 kpc (hereafter noted \minn{}) which we use as a proxy for the 
            \textit{in situ} stellar component. This is motivated both by recent observations and simulations 
            (e.g.~\citealt{vanDokkum2010}, \citealt{RodriguezGomez2016}). The value of 10 kpc that we quote here corresponds to the radius of the major axis of the isophotal ellipse (xxx check).
        \item stellar mass within 100 kpc (hereafter noted \mtot{}). We use \mtot{} as a proxy of the total stellar mass. In Paper I we showed that \mtot{} recovers more light compared to HSC \cmodel{} photometry with offsets that can be a large as 0.2 dex in magnitude.           
   \end{itemize}
   
   We use these two simple metric masses to explore the 
   \mstar{}-dependence of the fraction of accreted stars, and reveal to the diversity of 
   stellar envelopes among massive galaxies. In practice, we also have the full profiles for each galaxy and can also cast our results in terms of the full stellar mass profiles. However, in many cases we find it more useful to display figures using the more simple \minn{} and \mtot{} quantities. In Figure \ref{fig:m100_m10_color}, we highlight the diversity of galaxies as a function of \minn{} and \mtot{}. Figure \ref{fig:m100_m10_color} shows a random subsample of massive galaxies with very similar \minn{} that have large 
   range of \mtot{}. 
   We will use these two apertures masses to guide our comparisons of massive galaxies as a function of environments.  
...
\subsection{Massive Central Galaxies from Different Environments}
    ...
    In Paper~I, we demonstrate that this sample is almost mass complete over our full redshift range. In addition to the mass cut above, we also limit our sample to galaxies that live at the centers of their own 
    dark matter haloes-- so-called ``central'' galaxies. We use the \redm{} \texttt{v5.10} \citep{Rykoff2014, Rozo2015b} cluster catalog to construct a central galaxy sample.
    
    %Central galaxies are important to the study of galaxy-halo connection and follow stellar mass--halo mass relation (SHMR; e.g. \citealt{Leauthaud2012, 
    ...
    at the high end of our redshift window. The $P_{\mathrm{Cen}}$ limit is 
    selected to exclude low probability central galaxy candidates. 
    Based on the \mhalo{}-$\lambda$ relation calibrated by \citet{Simet2016}, these corresponds to central galaxies living in halos with \mhalo{}$>10^{14.2}$~\msun{}. This calibration is consistent with other works (e.g. \citealt{Saro2015, Farahi2016, 
    ...
    \citet{Diemer2015}. We further exclude all galaxies within a cylinder around each cluster, with a radius
    equal to $R_{\mathrm{200b}}$ and a length equal to twice the value of the photometric 
    redshift uncertainty of the cluster (typically around 0.015 to 0.025). This second sample will be dominated by central galaxies living in haloes with
    $M_{\mathrm{200b}} < 10^{14}$~\msun{}, and will refer to this sample as \nbcg{}. There are  833 galaxies with  
\logmtot{}$> 11.6$ in this sample.
...
%    Among them, 1564 (833) have \logmtot{}$> 11.5$ (11.6).
    ...
    Both of them follow the same ``red-sequence'' with only a handful of galaxies displaying bluer colors.
    
    We note that our method fails to extract 1-D profiles for $\sim10$\% of 
    \rbcg{} and \nbcg{} galaxies due on-going major mergers or projection effects (e.g.\ nearby foreground galaxy or bright stars). However, this small failure rate should not affect any of the results in this paper.
...
\section{Results}
    ...
    As shown in Figure~\ref{fig:m100_m10_color}, massive central galaxies at fixed  \minn{} display a large diversity in their stellar haloes. In Paper~1, we explored the \mstar{}-dependence of the stellar haloes in massive 
    galaxies, and concluded that the general trends are consistent with the 
    predictions of the two-phase formation scenario. In this paper, we now investigate the relation between galaxy profiles, their stellar halos, and dark matter halo mass. Here we use \mtot{} as proxy for the ``total'' \mstar{}
    of massive galaxies and we use \minn{} as a rough proxy for the mass of the 
    \textit{in situ} component.  We remind the reader that although a circular aperture is shown on 
    Fig~\ref{fig:m100_m10_color}, in practice we extract 1-D \mden{} profiles and estimate 
    \mtot{} and \minn{} using elliptical apertures following the average fluxed-weighted isophotal shape. 
    
    
    % We said this already...
    %Also,given the narrow \mstar
    %{} and redshift range, we ignore \mstar{} growth 
...
\subsection{Environmental Dependence of the Profiles of Massive Galaxies}
    ...
    First, we ask whether the \mden{} profiles of massive central galaxies depend on halo mass at fixed stellar mass. To ask this question, however, we must first define which stellar mass exactly is being fixed in the comparison. We will show results both when we fix \mtot{} as well as when we fix \minn{}. We also ensure that all comparisons are performed with a fixed underlying redshift distribution by mathcing samples in redshift in addition to stellar mass
    (please see Appendix~\ref{app:match}, Appendix~\ref{app:redshift}, and Fig~\ref{fig:match} for details). 
...
%% ------------------------------------------------------------------------------------ %%     
    Fig~\ref{fig:prof_1} compares the \mden{} profiles of massive central galaxies living in low mass dark matter haloes to those living in high mass dark matter haloes at fixed \mtot{} 
    (left panel) and at fixed \minn{} (right panel). This Figure shows the main result of this paper, namely that \textbf{the \mden{} profiles of 
    massive central galaxies show a clear dependence on dark matter halo mass at both fixed \mtot{} and 
    \minn{}.}

    We perform several statistical tests to demonstrate that the differences we see in the median 
    \mden{} profiles are significant. We also conduct a variety of tests that verify the robustness of these results with respect choices regarding \mtot{} bins, our $\lambda$ cut, the redshift range, and the choice of apertures used for the metric masses. Please see Appendix~\ref{app:robust} for further details.
   
    The key features in Figure Fig~\ref{fig:prof_1} are the following:
    
    \begin{itemize}
        
        \item For the \mtot{} matched sample, although there is a clear and robust difference between the two median \mden{} profiles, the overall difference is subtle is mainly only apparent on $R>50$ kpc scales. This may explain why previous attempts to detect this signal using shallower images were not successful. 
        \item At fixed \mtot{}, central galaxies in high mass dark matter haloes display shallower \mden{} profiles compared to those living in low mass dark matter haloes (they have a flatter inner \mden{} profiles and more significant outer stellar haloes. 
        \item The median \mden{} profiles of the \rbcg{} and \nbcg{} samples cross each 
            other at ${\sim} 15$-20 kpc, roughly also the scale of the effective radius $R_{\mathrm{e}}$.      
        \item Finally, the differences between the two sets of profiles becomes more prominent at higher \mtot{}. This can be seen further dividing the sample into two \mtot{} bins (see Fig~\ref{fig:prof_2} and discussion in 
            Appendix~\ref{app:robust}) and when we discuss the relation between \mtot{} and 
            \minn{} (see Fig~\ref{fig:scaling_relation}). This trend has potentially interesting implications \alexie{can we explain in one phrase?} that will be investigated in follow-up work. 
    \end{itemize}

\alexie{lets chat about the following two paragraphs}
...
\subsection{The Environmental Dependence of Scaling Relations}
    ...
    The previous section showed that the \mden{} profiles of massive galaxies show a dependence on the dark matter mass of their host halos. In this section, we now turn our attention to the more commonly studied mass size relation  (\mstar{}--$R_{\mathrm{e}}$). In addition, we will also consider halo mass dependence in the \mtot{}--\minn{} plane. 
...
\subsubsection{Mass-Size Relation}
    ...
    Despite numerous attempts, previous studies have failed to detect the dark matter halo mass dependence of the \mstar{}--$R_{\mathrm{e}}$ relation at low-$z$ 
    (e.g. \citealt{Weinmann2009, Nair2010, HCompany13, Cerbrian2014}; 
    but see \citealt{Yoon2017}). 
    
    The previous section clearly showed that the profiles of massive galaxies depend on the masses of their host dark matter halos. We now ask whether or not this dependence is also manifest when considered in terms of the more traditionally studied \mstar{}--$R_{\mathrm{e}}$ relation. However, when considering the mass-size relation, there is ambiguity and multiple approaches in terms of defining "sizes". This is especially true in the context of massive galaxies with extended stellar haloes (a reason why we prefer to show profiles directly as in the previous section). In this section, we still use \mtot{} as proxy of ``total'' \mstar{} and we adopt 
    the radius enclosing 50\% of luminosity \alexie{mass?} within 100 kpc ($R_{\mathrm{50}}$; derived 
    from the $i$-band curve-of-growth) as the ``size''. This definition of ``effective radius'' is more robust against structural details, 
    choice of model, and the background subtraction \alexie{compared to what?}. 
    Massive galaxies from this sample are large enough that the impact of seeing is not a concern.
