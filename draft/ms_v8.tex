...
\subsubsection{General Trends and Comparison with Previous Work}
    ...
    \citep{Huang2013a} derived \mden{} profiles for a small sample of very nearby 
    ...
    The \citep{Huang2013a} profiles show good agreement with ours in the region of 
    ...
    \citep{Patel2013} profile with the one in our lowest stellar mass bin. 
...
%% ------------------------------------------------------------------------------------ %% 
    ...
    This is consistent with previous 2-D modeling results from \citep{Huang2013a}. 
...
\subsubsection{Mass-Size Relation}
    ...
    Despite numerous previous attempts, previous works have failed to clearly 
    ...
    By using the deep HSC images, and by comparing the \mden{} profiles directly, 
    we have shown that the structures of massive galaxies do depends on the masses of 
    their host halos (see the left panel of Fig~\ref{fig:scaling_relation}). 
    We now couch our results in terms of the more traditional "mass-size" relation. 
    For this purpose, we will use \mtot{} as our ``total'' \mstar{} and the radius 
    enclosing 50\% of luminosity within 100 kpc ($R_{\mathrm{50}}$; derived from $i$-band 
    curve-of-growth) as our ``size''. 
    ...
    Our massive galaxies are large enough so we do not need to worry about the 
    effects of seeing.
...
\subsection{The ``Two phase'' Formation Scenario of Massive Galaxies and the 
    ...
    %environments. The \mhalo{}-dependence that wecan be caused by many different physical processes. 
