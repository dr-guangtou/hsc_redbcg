...
\begin{abstract} 
    ...
    These extended stellar halos are key fossil records of the assembly history of 
    massive galaxies.  
    Using data that is both wide (${\sim}100$ deg$^2$) and deep ($>28.5$ \sb{} in 
    the $i$-band), we present a systematic study of the stellar halos of the most 
    massive galaxies (\mstar{}$\geq 11.5$) using a statistically significant sample 
    of more than $7000$ galaxies at $0.3 < z < 0.5$. 
    Our study is based on high-quality (0.6\asec seeing) imaging data from the Hyper 
    Suprime-Cam (HSC) survey which reaches ${\sim}4$ magnitudes fainter than SDSS in 
    surface brightness and enables us to directly trace mass profiles to 100 kpc 
    without resorting to stacking. 
    Shallow images and/or over-simplified models like \texttt{CModel} cannot capture 
    the extended stellar halo and result in underestimated stellar masses. 
    Our extended profiles reveal a number of mass-dependent trends. 
    We find that the halos of massive galaxies have higher ellipticity, and that the 
    outer ellipticity increases with stellar mass, but we find no evidence for trend 
    between stellar mass and color gradients. 
    As in previous work, we see a clear trend towards more extended outer profiles in 
    ...
    massive galaxies in which outer envelopes are built up at late times from series
    of mergers. 
    We provide surface mass density profiles in a convenient tabulated format to 
...
%% ------------------------------------------------------------------------------------ %% 
  \begin{figure*}
      ...
      \caption{
          ...
          line).  
          The grey shaded region shows the resampling error on the HSC SMF plus a 20\%
          uncertainties to account for other issues in our sample. 
...
\subsection{Impact of Missing Light on the Galaxy Stellar Mass Function}
    ...
    We do not include satellite galaxy in our sample (could affect ${\sim}10$\% of 
    galaxies at this very high-mass end) and can not extract 1-D \mden{} profile 
    for a small fraction of galaxies due to severe contamination of nearby object. 
    Therefore, we should expect at least ${\sim}20$\% uncertainties in the current 
    volume density distributions (shown as the shaded region on Fig \ref{fig:smf}).
...
%% ------------------------------------------------------------------------------------ %% 
	...
	the concentration galaxies with $11.0<$\logms{}$<11.4$ from \citealt{DSouza2015}).
	As expected, the ellipticity profile from image stacking method tends to be 
	shallower than the HSC one.  
	Uncertainties in aligning galaxies before stacking and the intrinsic isophotal 
	twist can lead to this effect. 
	...
	and slow-rotating nature of massive ETGs (e.g. \citealt{Wu2014}), so the 
    ...
    The $i$-band PSF has more prominent wing than the other bands that 
    artificially distributes more flux to the outskirt, and lead to redder color in 
...
\subsection{The Formation of Massive Galaxies and the Assembly of Outer Halos}
    ...
    The more elongated outer halo and the trend between ellipticity profile and 
    stellar mass may reflect that the orbital properties of infalling satellite are 
    not random. 
    In simulations, satellite orbit becomes more radial when it falls into more 
    massive halos and when the satellite-central mass ratio is high (e.g. 
    \citealt{Murante2007, Wetzel2011, Jiang2015}.
    In principle, these results help explain the observed trend, and emphasize the 
    importance of major merger in shaping the halo of massive galaxies. 
