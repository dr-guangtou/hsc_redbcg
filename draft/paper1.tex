...
%% ------------------------------------------------------------------------------------ %% 
    ...
    the HSC data, 
    (b) examine the implications for the stellar mass function when the outer parts 
    of galaxies are accounted for, and
    (c) look at trends in outer envelope properties as a function of stellar mass. 
    In a second paper in this series, we will directly show the environmental dependence
    of the outer envelope mass (Huang\etal. in prep.)
    }
...
\subsection{Initial Massive Galaxy Sample}
    ...
    Based on \citet{Leauthaud2016}, galaxies with \logms{}$\geq 11.5$ mostly have 
    ...
    spec-$z$. Before we can define our mass-complete sample of central galaxies required for 
    our analysis, we describe our one-dimensional photometric analysis (\S \ref{sec:ellipse}) 
    and stellar mass estimations (\S \ref{sec:mstar}). We then define the final sample in \S \ref{sec:final}.
...
\section{Measurements of 1-D Surface Brightness Profiles}
    ...
    \jenny{This cut removes roughly 10\% of the sample.}
    ...
    \jenny{Describe the two apertures here I think?}
    Given our exquisite profiles out to 100 kpc, the meaning of ``total'' magnitude can take on different meanings. 
    Throughout this work, we consider two benchmark physical apertures:
        \begin{itemize}    
        \item The stellar mass within 10 kpc (hereafter noted \minn{}). 
            According to the two-phase scenario, the ``in-situ'' star-formation quickly 
            builds up the inner, dense core of massive ETGs.  
