...
\begin{abstract} 
    ...
    With these deeper data, we show definitively that at fixed stellar mass,
    massive central galaxies display mass density profiles that are not self similar, 
    with the accreted fraction increasing towards unity for galaxies in our highest 
    stellar mass bin. 
    ...
    formation.
    \song{First trial by Jenny; need more works}
...
\section{Introduction}
    ...
    so-called ``red nuggets'' (e.g. \citealt{Damjanov2009}) which have smaller 
    effective radii ($R_{\mathrm{e}}$; e.g. \citealt{Trujillo2006, vanDokkum2008, 
    Cimatti2008}), slightly higher central velocity dispersion and stellar mass 
    density (\mden{}; e.g. \citealt{vandeSande2011, Belli2014}), more disk-like 
    morphologies (e.g. \citealt{vanderWel2011}) than their local descendants 
    ...
    massive galaxies and a large fraction of these massive progenitors become 
    quiescent at $z>1$ (e.g. \citealt{Bezanson2009, Kriek2016}). 
    ...
    \update{
    To explain the observed significant increase of $R_{\mathrm{e}}$ (e.g. 
    ...
    Laporte2013}).
    More importantly, both numerical simulation (e.g. \citealt{Oser2010}) and 
    semi-analytic model (SAM; e.g. \citealt{LeeYi2013, LeeYi2017}) agree that the 
    mass fraction of the accreted component should increase with total stellar mass, 
    although different models still predict somewhat different values at fixed 
    stellar mass (e.g. \citealt{Lackner2012, Cooper2013, Qu2017}).
    According to the recent results from Illustris \footnote{
    \texttt{http://www.illustris-project.org/}} simulation 
    (\citealt{Vogelsberger2014}, \citealt{Genel2014}), the fraction of the 
    accreted mass increases significantly with total stellar mass at \logms{}$>11.0$, 
...
%% ------------------------------------------------------------------------------------ %% 
    ...
    \update{
    At the same time, some of these ``two-phase'' scenario's predictions still have 
    little observational constraint. 
    One-dimensional analysis used to conclude that \ser{} law can reasonably describe  
    the surface brightness (or stellar mass density) profiles of nearby massive 
    elliptical galaxies (e.g. \citealt{Kormendy2009}; except for the most central 
    region), and the \ser{} index seems to increase with total luminosity (or stellar 
    mass; e.g. \citealt{Graham2013}). 
    There was also suggestion that elliptical galaxies can be divided into two groups
    based on how well their 1-D surface brightness profiles can be described by \ser{}
    law (e.g. \citealt{Schombert2015}). 
    Meanwhile, recent two-dimensional analysis points out that the stellar distributions 
    of massive ETGs are better fit by multiple-component models 
    (e.g. \citealt{Huang2013a, Oh2017}). 
    However, it is still not clear whether these 1-D or 2-D results are consistent with 
    the ``two-phase'' scenario. 
    One of the main limitation here is that one needs very deep observations for large 
    sample of massive ETGs to correctly estimate their total stellar mass 
    (e.g. \citealt{Bernardi2013, DSouza2014}) and study the assembly history via 
    their accreted halos (e.g. \citealt{Capaccioli2015, Iodice2016, Iodice2017}).
    Currently, such works are often limited by the shallow images such as the ones from
    the Sloan Digital Sky Survey (SDSS; e.g. \citealt{SDSSDR7, SDSSDR12}) or the small 
    sample size. 
    }
    ...
    $0.3 < z < 0.5$ using ${\sim} 100$ square degree of data from the 
    HSC wide layer. 
    In this paper, we use this sample to 
    (a) measure individual \mden{} profiles for galaxies out to 100 kpc reliably with 
    the HSC data, 
    (b) examine the implications for the stellar mass function (SMF) when the outer 
    parts of galaxies are accounted for, and
    (c) investigate the dependence of their outer envelopes on stellar mass.    
    ...
    %\jenny{I suggest removing this here. I do not think it belongs in the Intro:
    ...
    %\song{Since the original paper is quite long and complicated, we decide to define
    %the two aperture masses in the beginning, and explain why they are important.
    %For the new draft, I think it is Ok to do this later, but more comments on this 
    %are welcomed!}
...
\subsection{The Hyper Suprime-Cam Survey}
    ...
    perform large statistical study of the surface brightness profiles of ETGs out to 
    large radii.     
...
\subsection{Initial Massive Galaxy Sample}
    ...
    and keep the matched objects with useful red-sequence photo-$z$ 
    ($0.3 \leq z_{\lambda} < 0.5$) in the final sample. 
    These galaxies are the best candidates for BCGs in clusters selected from 
    ...
    spec-$z$. 
    Before we can define our mass-complete sample of central galaxies required for 
...
%          (red lines).
%          \song{New figure for completeness; Need to update the caption}
%          }
...
%% ------------------------------------------------------------------------------------ %% 
  \begin{figure}
      \centering 
      \includegraphics[width=\columnwidth]{fig/redbcg_mass_diff}
      \caption{
          \song{New figures to show how \mstar{} changes with different definitions; 
...
\section{Measurements of 1-D Surface Brightness Profiles}
    ...
    Massive ETGs imaged at the depth of HSC are not well modeled by the de~Vaucouleurs
    or \ser law. 
    These models will fail to simultaneously describe the profile in both the inner and 
    the outer regions and also cannot account for any radial variations in ellipticity 
    and position angle. 
    ...
    Background subtraction uncertainty also affects the 2-D model fitting dramatically, 
...
%% ------------------------------------------------------------------------------------ %% 
    ...
    \update{
    Given our exquisite profiles out to 100 kpc, the definition and meaning of ``total'' 
    magnitude (and stellar mass) becomes more interesting.
    With the help of the \m2l{} estimated in the next section (\S \ref{sec:mstar}), 
    we can integrate the profile out to certain radii, and estimate the stellar mass 
    within.  
    ...
    \begin{itemize} 
        ...
        \item \textbf{The \mstar{} within the inner 10 kpc} (hereafter noted \minn{}). 
            According to the two-phase scenario, the ``in-situ'' star-formation quickly 
            builds up the inner, dense core of massive ETGs.  
            Based on recent observation (e.g.~\citealt{vanDokkum2010}) and simulations 
            (e.g. \citealt{RodriguezGomez2016}), the in-situ component dominates the 
            \mstar{} within one effective radius ($R_{\mathrm{e}}$, or 5-10 kpc) of 
            $z{\sim}0$ massive ETGs.
            We therefore use \minn{} as proxy for the mass forged during the ``in-situ''
            phase. 
            Given the quality of the HSC data, we can reliably measure \minn{} over our 
            redshift range ($1.0^{\arcsec}$ in radius equals 4.4 and 6.1 kpc at redshift
            0.3 and 0.5).  
            
        \item \textbf{The stellar mass within 100 kpc} (hereafter noted \mtot{}). 
            For our galaxy sample, a 100 kpc aperture corresponds to 5-10 
            $\times R_{\mathrm{e}}$. 
            We show in section \ref{ssec:mtotal} that most of the \mstar{} for these 
            ETGs can be captured by a 100 kpc radius and that \mtot{} is a good proxy 
            for the ``total'' \mstar{}. 
            Although not perfect, we argue that our measurements of \mtot{} (which are 
            actually measuring the light directly out to 100 kpc) are a better tracer 
            of total \mstar{} than model-dependent results from shallower data 
...
\subsection{Stellar Masses from SED Fitting}
    ...
    (the highest value allowed by FSPS) \jenny{define metallicity here, means different things sometimes}. 
    ...
    \cmodel{} for a bright galaxy is $\sim 1$\% in the $r$, $i$, $z$ bands, 
...
\subsection{``Total'' Stellar Masses}
    ...
    stellar mass. \jenny{Here is where we need discussion of Figure \ref{fig:mass_diff} I think? When do 
    we discuss cModel magnitudes?}
	\jenny{Check me here!}
...
%% ------------------------------------------------------------------------------------ %% 
    ...
    \jenny{Could we add a panel to the plot showing the mass bins as above with the ellipticity profiles?}
...
%% ------------------------------------------------------------------------------------ %% 
  \begin{figure*}
      ...
      \caption{
          \song{New figures to illustrate the assumption of in-situ and accreted mass;
                Need to update the caption.}
...
%% ------------------------------------------------------------------------------------ %% 
  \begin{figure*}
      ...
      \caption{
          \song{New figures to show the impact of mass definition on SMF. 
                Need to update the caption.}
...
\song{A few appendix need to be moved to Paper II}
%% ------------------------------------------------------------------------------------ %% 
...
\begin{figure*}
    ...
    \caption{
        ...
        The format is very similar to the left panel of Fig~\ref{fig:mass_complete}. 
