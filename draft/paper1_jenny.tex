...
\section{Introduction}
    ...
    density (\mden{}; e.g. \citealt{vandeSande2011, Belli2014}), and more disk-like 
    ...
    Laporte2013}).
    Both numerical simulation (e.g. \citealt{Oser2010}) and 
    semi-analytic modeling (SAM; e.g. \citealt{LeeYi2013, LeeYi2017}) agree that the 
    mass fraction in the accreted component should increase with total stellar mass, 
    although details still differ between models (e.g. \citealt{Lackner2012, Cooper2013, Qu2017}).
    According to recent results from the Illustris \footnote{
    \texttt{http://www.illustris-project.org/}} simulation 
    (\citealt{Vogelsberger2014}, \citealt{Genel2014}), the fraction of  
...
%% ------------------------------------------------------------------------------------ %% 
    ...
    \update{
    Given the success of this two-phase scenario in explaining trends like overall size growth, it is 
    time to confront their predictions with additional observations using local galaxy mass profiles. Some 
    progress has been made. One-dimensional analysis has shown that a \ser{} law well-describes 
    the surface brightness (or stellar mass density) profiles of most nearby massive 
    elliptical galaxies (e.g. \citealt{Kormendy2009}; except for the most central 
    region), with the \ser{} index increasing with total luminosity (e.g. \citealt{Graham2013}).
	But, the situation may be more complicated, with a suggestion from \citet{Schombert2015} that there are actually 
    two families of light profiles. Two-dimensional analysis, meanwhile, finds that the stellar distributions 
    of massive ETGs are better fit by multiple-component models 
    (e.g. \citealt{Huang2013a, Oh2017}). 
    To really confront the two-phase model requires very deep observations for large 
    samples of massive ETGs to correctly estimate their total stellar mass 
    (e.g. \citealt{Bernardi2013, DSouza2014}) and study their assembly history via 
    their accreted halos (e.g. \citealt{Capaccioli2015, Iodice2016, Iodice2017}).
    Currently, such work is limited by the depth of imaging from large-area surveys such as the 
    Sloan Digital Sky Survey (SDSS; e.g. \citealt{SDSSDR7, SDSSDR12}) or the small 
    area of deeper imaging. 
    }
...
%% ------------------------------------------------------------------------------------ %% 
    ...
    \update{
    Given our exquisite profiles that extend to 100 kpc, the definition and meaning of ``total'' 
    magnitude (and stellar mass) becomes more nuanced.
